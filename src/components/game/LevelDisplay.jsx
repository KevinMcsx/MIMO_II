...
export default function LevelDisplay({ level, xp, compact = false }) {
  ...
  return (
    <div className="bg-white/90 dark:bg-slate-900/90 backdrop-blur-sm rounded-lg sm:rounded-xl p-2 sm:p-4 shadow-lg border border-purple-300 dark:border-purple-700 sm:border-2 max-w-[180px] sm:max-w-none">
      <div className="flex items-center justify-between mb-1 sm:mb-2">
        <div className="flex items-center gap-1 sm:gap-2">
          ...
          <div>
            <p className="font-bold text-slate-800 dark:text-slate-100 text-xs sm:text-base">{t('level')} {level}</p>
            <p className="text-[10px] sm:text-xs text-slate-600 dark:text-slate-300">{progress.current} / {progress.needed} XP</p>
          </div>
        </div>
      ...
      <p className="text-[10px] sm:text-xs text-slate-500 dark:text-slate-400 text-center mt-0.5 sm:mt-1">{progress.percentage}% {t('toNextLevel')}</p>
    </div>
  );
}