...
export default function Game33NumberSequence({ difficulty, onMainMenu, playerName }) {
  ...
  const ds = { 1: { rounds: 10, roundTime: 8, maxStart: 10, maxStep: 3, mult: 1 }, 2: { rounds: 12, roundTime: 7, maxStart: 15, maxStep: 4, mult: 1.5 }, 3: { rounds: 14, roundTime: 6, maxStart: 20, maxStep: 5, mult: 2 }, 4: { rounds: 16, roundTime: 5, maxStart: 25, maxStep: 7, mult: 3 } };
  ...
  const generateRound = () => {
    const start = rand(1, s.maxStart);
    const step = rand(1, s.maxStep);
    const len = 4;
    const arr = Array.from({ length: len }, (_, i) => start + i * step);
    const idx = rand(0, len - 1);
    const correct = arr[idx];
    const opts = new Set([correct]);
    while (opts.size < 4) { const d = correct + rand(-step - 2, step + 2); if (d >= 0 && d !== correct) opts.add(d); }
    setSeq(arr); setMissingIdx(idx); setAnswer(correct); setOptions(shuffle([...opts]));
  ...
  const endGame = async () => {
    ...
    await saveGameResult({ game_type: 33, difficulty, total_time: totalTime, avg_reaction_time: 0, correct_hits: correctCount, wrong_hits: wrongCount, correct_shapes: correctCount, wrong_shapes: wrongCount, score, player_name: playerName });
  ...
  if (gameState === 'finished') {
    const totalTime = Math.round(round * s.roundTime * 1000);
    return <ResultsScreen gameTitle={t('numberSequence')} gameResult={{ game_type: 33, difficulty, score, avg_reaction_time: 0, correct_hits: correctCount, wrong_hits: wrongCount, total_time: totalTime }} stats={{ totalTime, avgReactionTime: 0, correctHits: correctCount, wrongHits: wrongCount, totalAttempts: correctCount + wrongCount }} onPlayAgain={() => window.location.reload()} onMainMenu={onMainMenu} playerName={playerName} />;
  }
    ...
    <div className="min-h-screen flex items-center justify-center p-4">
      <div className="bg-white/90 backdrop-blur-md rounded-3xl shadow-2xl p-8 max-w-2xl w-full relative">
        <div className="flex justify-between items-center mb-6">
          <div><p className="text-2xl font-bold text-slate-800">{t('round')} {round}/{totalRounds}</p><p className="text-lg font-semibold text-violet-600">{t('score')}: {score}</p></div>
        ...
        <div className="mb-4 text-center"><p className="text-sm font-semibold text-slate-500 uppercase">What's the missing number?</p></div>
        {seq.length > 0 && (
          <div className="flex justify-center items-center gap-3 mb-8">
            {seq.map((n, i) => (
              <div key={i} className={`w-16 h-16 rounded-2xl border-2 flex items-center justify-center text-3xl font-black ${i === missingIdx ? 'bg-violet-100 border-violet-400 text-violet-400' : 'bg-slate-50 border-slate-200 text-slate-800'}`}>{i === missingIdx ? '?' : n}</div>
            ))}
          </div>
        )}
        {gameState === 'playing' && (
      ...
      <TutorialModal isOpen={showTutorial} onClose={() => setShowTutorial(false)} gameId={33} />
    </div>
  );
}