...
export default function Game29FindMax({ difficulty, onMainMenu, playerName }) {
  ...
  const ds = { 1: { gridSize: 4, rounds: 10, roundTime: 6, maxNum: 50, mult: 1 }, 2: { gridSize: 5, rounds: 12, roundTime: 5, maxNum: 80, mult: 1.5 }, 3: { gridSize: 5, rounds: 14, roundTime: 4, maxNum: 99, mult: 2 }, 4: { gridSize: 6, rounds: 16, roundTime: 3, maxNum: 99, mult: 3 } };
  ...
  const generateRound = () => {
    const nums = Array.from({ length: total }, () => rand(1, s.maxNum));
    const mv = Math.max(...nums);
    setGrid(nums);
    setMaxVal(mv);
  ...
  const endGame = async () => {
    ...
    await saveGameResult({ game_type: 29, difficulty, total_time: totalTime, avg_reaction_time: 0, correct_hits: correctCount, wrong_hits: wrongCount, correct_shapes: correctCount, wrong_shapes: wrongCount, score, player_name: playerName });
  ...
  if (gameState === 'finished') {
    const totalTime = Math.round(round * s.roundTime * 1000);
    return <ResultsScreen gameTitle={t('findMax')} gameResult={{ game_type: 29, difficulty, score, avg_reaction_time: 0, correct_hits: correctCount, wrong_hits: wrongCount, total_time: totalTime }} stats={{ totalTime, avgReactionTime: 0, correctHits: correctCount, wrongHits: wrongCount, totalAttempts: correctCount + wrongCount }} onPlayAgain={() => window.location.reload()} onMainMenu={onMainMenu} playerName={playerName} />;
  }
    ...
    <div className="min-h-screen flex items-center justify-center p-4">
      <div className="bg-white/90 backdrop-blur-md rounded-3xl shadow-2xl p-8 max-w-2xl w-full relative">
        <div className="flex justify-between items-center mb-6">
          <div><p className="text-2xl font-bold text-slate-800">{t('round')} {round}/{totalRounds}</p><p className="text-lg font-semibold text-amber-600">{t('score')}: {score}</p></div>
        ...
        <div className="mb-4 text-center"><p className="text-sm font-semibold text-slate-500 uppercase">Tap the highest number</p></div>
        {gameState === 'playing' && (
          <div className="grid gap-2 mb-6" style={{ gridTemplateColumns: `repeat(${s.gridSize}, 1fr)` }}>
            {grid.map((val, i) => <button key={i} onClick={() => handleClick(val)} className="aspect-square bg-amber-50 rounded-lg border border-amber-200 hover:border-amber-400 flex items-center justify-center text-lg sm:text-xl font-bold text-slate-700 transition-all">{val}</button>)}
          </div>
        )}
      ...
      <TutorialModal isOpen={showTutorial} onClose={() => setShowTutorial(false)} gameId={29} />
    </div>
  );
}